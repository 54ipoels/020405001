...
//-------- update data pegawai --------------//
	...
	# rekapitulasi kekuatan SDM
	function count_pegawai_by_unit_status($unit,$status)
	{
		$query = "
				SELECT * 
				FROM v3_pegawai a
				LEFT JOIN v3_peg_tmt b ON b.p_tmt_nipp = a.peg_nipp
				LEFT JOIN v3_peg_unit c ON c.p_unt_nipp = a.peg_nipp
				WHERE b.id_peg_tmt = ( SELECT MAX( k.id_peg_tmt )FROM v3_peg_tmt k WHERE k.p_tmt_nipp = a.peg_nipp ) 
				AND c.p_unt_tmt_start = ( SELECT MAX( l.p_unt_tmt_start )FROM v3_peg_unit l WHERE l.p_unt_nipp = a.peg_nipp AND p_unt_tmt_end = '0000-00-00' ) 
				AND p_tmt_status =  '$status'
				AND p_tmt_end =  '0000-00-00'
				AND p_unt_kode_unit =  '$unit'
				";
		$query = $this->db->query($query); 
		return $query->num_rows();
	...
	function count_pegawai_mpp_by_unit($unit)
	{
		$tahun = date('Y');
		$type=54;
		$limit=56;
		...
		$tahun2=$tahun-1;
		$query = ("
			SELECT * FROM v3_pegawai a
			LEFT JOIN v3_peg_tmt b ON b.p_tmt_nipp = a.peg_nipp
			LEFT JOIN v3_peg_unit c ON c.p_unt_nipp = a.peg_nipp
			WHERE b.id_peg_tmt = ( SELECT MAX( k.id_peg_tmt )FROM v3_peg_tmt k WHERE k.p_tmt_nipp = a.peg_nipp ) 
			AND c.p_unt_tmt_start = ( SELECT MAX( l.p_unt_tmt_start )FROM v3_peg_unit l WHERE l.p_unt_nipp = a.peg_nipp AND p_unt_tmt_end = '0000-00-00' ) 
			AND ( $tahun - YEAR(a.peg_tgl_lahir) > $type  AND $tahun - YEAR(a.peg_tgl_lahir) < $limit 	AND MONTH(a.peg_tgl_lahir) <> 12 ) 
			OR (MONTH(a.peg_tgl_lahir) = 12 AND DATE(a.peg_tgl_lahir) > 1 AND  $tahun2 - YEAR(a.peg_tgl_lahir) > $type AND $tahun2 - YEAR(peg.peg_tgl_lahir) < $limit )
			AND peg_tmt.p_tmt_end = '0000-00-00'
			AND p_tmt_end =  '0000-00-00'
			AND p_unt_kode_unit =  '$unit'
			" );
